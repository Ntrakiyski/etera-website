...
- One featured people record.
- One featured partner with related logo media.
- Published content for Home, Work, The Atelier, Services, and Contact.
- Site Settings used by the home-page footer.
...
| Users | List, create temporary editor, read, update role, delete | Pass |
...
| Responsive rendering | Desktop and 390px mobile, no horizontal overflow, no broken images | Pass |
...
- Build output currently contains non-blocking warnings from bundled third-party Payload/OpenNext code. Dependency installation also reports seven transitive audit findings (one low and six moderate); review these during routine dependency maintenance rather than applying an unreviewed automatic upgrade.
